...
```
             ...
             raw_books -> validate -> transform -> load -> embeddings
                  ...
                  FastAPI REST API (port 8000)
                   /auth  /books  /reviews  /genres
                   /ratings  /recommendations  /users
                          |
                  React SPA (port 5173)
                  Vite + TS + Tailwind v4
```
...
3. staging_data_sensor detects new crawl_job_id -> triggers etl_pipeline
4. Dagster reads staging table -> validates -> transforms -> upserts to PostgreSQL -> generates embeddings
5. FastAPI serves paginated books, reviews, genres, ratings, recommendations
6. React SPA fetches via /api proxy -> renders browse/detail/auth/profile/ratings pages
7. Users rate books -> stored in PostgreSQL -> powers embedding-based recommendations
```
...
dagster  -> raw_books_staging (read) + PostgreSQL (write) + backend.db.models (lazy import)
backend  -> PostgreSQL (read/write) + pgvector (cosine distance) + .env (config)
...
```
User rates >= 5 books -> compute weighted user embedding (rating-2 weight)
pgvector cosine distance -> nearest books -> personalized recommendations
User has < 5 ratings -> fallback to popular books (by avg critic rating)
Supports category filtering (fiction/nonfiction) and genre/subgenre filtering
```
