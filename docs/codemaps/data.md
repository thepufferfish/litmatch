...
|-------|-----|-------------|-------------------|
| book | id | title, author_id, publisher_id, publish_date, description, url, cover, is_fiction, last_scraped | url |
| author | id | name | name |
...
| Table | PK | Key Columns | Unique Constraints |
|-------|-----|-------------|-------------------|
| review | id | book_id, critic_id, publication_id, rating (int 1-4), review (text), url | url |
| critic | id | name | name |
...
| user_rating | id | user_id, book_id, rating (int 1-5), created_at, updated_at | — |
